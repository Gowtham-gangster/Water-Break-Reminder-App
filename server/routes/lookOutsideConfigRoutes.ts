...
export const lookOutsideConfigRouter = Router();

// Get Look Outside Configuration (Strictly scoped to authenticated user)
lookOutsideConfigRouter.get('/', requireAuth, (req: AuthenticatedRequest, res) => {
  const userId = req.user!.id;
  const config = db.getLookOutsideConfigByUserId(userId) || db.upsertLookOutsideConfig(userId, {});
  res.json({ config });
});

const TIME_REGEX = /^([01]\d|2[0-3]):([0-5]\d)$/;

// Update Look Outside Configuration (Strictly scoped to authenticated user)
lookOutsideConfigRouter.put('/', requireAuth, (req: AuthenticatedRequest, res) => {
  const userId = req.user!.id;
  ...
  if (duration_seconds !== undefined) {
    if (typeof duration_seconds !== 'number' || duration_seconds <= 0 || !Number.isFinite(duration_seconds)) {
      return res.status(400).json({ error: 'Break duration must be a positive number greater than 0' });
    }
  }
  ...
  const currentConfig = db.getLookOutsideConfigByUserId(userId);
  ...
  const config = db.upsertLookOutsideConfig(userId, {
